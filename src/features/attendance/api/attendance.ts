...
export const attendanceApi = {
  enroll: (file: File, params: Record<string, string>) => {
    ...
    return multipartClient.post<EnrollResponse>(`${BASE}/enroll/`, form).then((r) => r.data)
  },
  ...
  updateEmbedding: (file: File, params: Record<string, string>) => {
    ...
    Object.entries(params).forEach(([k, v]) => v && form.append(k, v))
    return multipartClient
      .post<UpdateEmbeddingResponse>(`${BASE}/update-embedding-via-period/`, form)
      .then((r) => r.data)
  },
  ...
  getEnrollmentStats: () =>
    apiClient.get<EnrollmentStatsResponse>(`${BASE}/enrollment-stats/`).then((r) => r.data),

  viewStudents: (params: Record<string, string>) =>
    apiClient
      .get(`${BASE}/view-students/${buildQueryString(params)}`, { responseType: 'blob' })
      .then((r) => r.data as Blob),

  getAttendanceOnDate: (params: Record<string, string>) =>
    apiClient
      .get<AttendanceDateResponse>(`${BASE}/view-attendance-on-date/${buildQueryString(params)}`)
      .then((r) => r.data),

  getAttendanceRange: (params: Record<string, string>) =>
    apiClient
      .get<AttendanceRangeResponse>(`${BASE}/view-attendance-range/${buildQueryString(params)}`)
      .then((r) => r.data),

  getChangeLog: (params: Record<string, string>) =>
    apiClient
      .get<ChangeLogResponse>(`${BASE}/database-change-log/${buildQueryString(params)}`)
      .then((r) => r.data),

  deleteStudent: (params: Record<string, string>) =>
    apiClient
      .delete<DeleteResponse>(`${BASE}/delete-student/${buildQueryString(params)}`)
      .then((r) => r.data),

  deleteClass: (params: Record<string, string>) =>
    apiClient
      .delete<DeleteResponse>(`${BASE}/delete-class/${buildQueryString(params)}`)
      .then((r) => r.data),

  deleteBulkFromBoth: (params: Record<string, string>) =>
    apiClient
      .delete<DeleteResponse>(`${BASE}/delete-bulk-from-both/${buildQueryString(params)}`)
      .then((r) => r.data),
}
