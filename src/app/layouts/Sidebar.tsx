...
/** Small count pill for nav items; caps at 99+ and hides at 0. */
            ...
            <div className="flex-1 overflow-y-auto px-3 pb-3 pt-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              <nav key={activeCategory?.label} className="space-y-px">
                {activeCategory?.children?.map((child, i, arr) => {
                  // Group heading renders once, above the first child that
                  // carries a new group value (children are contiguous by group).
                  const showGroup =
                    ...
                    <div key={child.href || child.label}>
                      ...
                      <NavLink
                        to={child.href || "#"}
                          ...
                          child.end !== undefined ? child.end : child.href === "/"
                        }
