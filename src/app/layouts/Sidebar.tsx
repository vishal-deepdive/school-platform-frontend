...
export function Sidebar({ mobile, onClose }: SidebarProps) {
  ...
  return (
    <div className="flex h-full relative">
      ...
      <aside
        className={cn(
          "relative z-20 flex h-full w-16 flex-col items-center bg-background py-4",
      ...
      {isCollapsed && hasSecondary && (
        <button
          ...
          className="absolute left-[52px] top-16 z-30 flex h-6 w-6 items-center justify-center rounded-full border border-border bg-background text-muted-foreground shadow-sm hover:bg-accent hover:text-foreground transition-all duration-200 cursor-pointer"
      ...
      <aside
        className={cn(
          "z-10 h-full bg-background/80 transition-all duration-300 ease-in-out overflow-hidden",
          hasSecondary && !isCollapsed
            ? "w-60"
            : "w-0"
        )}
      >
          ...
          <div className="flex h-full w-60 flex-col">
            <div className="flex h-16 shrink-0 items-center justify-between pl-5 pr-3">
              <div className="min-w-0">
                <h2 className="truncate text-md font-semibold tracking-tight text-foreground">
                  {activeCategory?.label}
                </h2>
              </div>
              <button
                onClick={() => setIsCollapsed(true)}
                className="shrink-0 rounded-lg p-1.5 text-muted-foreground/70 hover:bg-muted/60 hover:text-foreground hover:text-primary transition-colors duration-200"
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              <nav key={activeCategory?.label} className="space-y-1">
                {activeCategory?.children?.map((child, i) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      cn(
                        "group relative flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition-all duration-200 animate-nav-item-in",
                        isActive
