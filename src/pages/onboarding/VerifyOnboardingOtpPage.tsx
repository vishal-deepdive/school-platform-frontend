...
export function VerifyOnboardingOtpPage() {
  ...
  const state = location.state as { applicationId?: string; email?: string } | null
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`/onboarding/status?id=${encodeURIComponent(applicationId.trim())}`)
    } catch (err) {
  ...
  return (
    <div className="flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8 text-center sm:text-left">
        ...
        <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Verify School Email</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Enter the 6-digit code sent to your email to verify your onboarding application.
        </p>
      </div>
      ...
      <form onSubmit={handleVerify} className="flex flex-col gap-5" noValidate>
        ...
        <div className="pt-2">
          <AuthButton type="submit" disabled={isVerifying} className="w-full mt-2">
        ...
        <p className="text-center text-sm text-muted-foreground mt-2">
          <Link to="/onboarding/apply" className="font-semibold text-primary hover:text-primary/80 transition-colors">
            Back to Application
          </Link>
        </p>
      </form>
