...
import { Building2, MapPin, GraduationCap, FileText, User, ChevronRight, ChevronLeft, Check, Loader2 } from 'lucide-react'
...
// ── Constants ─────────────────────────────────────────────────────────────────
const STEPS: { id: StepIndex; title: string; subtitle: string; icon: React.ElementType }[] = [
  { id: 1, title: 'School Info',       subtitle: 'Basic details',        icon: Building2 },
  { id: 2, title: 'Contact & Address', subtitle: 'Location & contact',   icon: MapPin },
  { id: 3, title: 'Academic Details',  subtitle: 'Curriculum & size',    icon: GraduationCap },
  { id: 4, title: 'Documents',         subtitle: 'Certificate upload',   icon: FileText },
  { id: 5, title: 'Principal Account', subtitle: 'Login credentials',    icon: User },
]

const STEP_FIELDS: Record<StepIndex, FieldPath<SchoolOnboardingFormData>[]> = {
  1: ['school_name', 'board', 'other_board', 'school_type', 'other_school_type'],
  2: ['email', 'mobile', 'address_line_1', 'city', 'state', 'pin_code'],
  3: ['student_count', 'classes_from', 'classes_to', 'medium_of_instruction', 'other_medium_of_instruction'],
  4: [],
  5: ['principal_name', 'principal_email', 'principal_password', 'confirm_password', 'terms'],
}
...
export function SchoolOnboardingPage() {
  ...
  const [submitted, setSubmitted] = useState<OnboardingApplicationResponse | null>(null)
  ...
  const onSubmit = async (data: SchoolOnboardingFormData) => {
    ...
    try {
      ...
      if (data.other_board?.trim()) fd.append('other_board', data.other_board.trim())
      fd.append('school_type', data.school_type)
      if (data.other_school_type?.trim()) fd.append('other_school_type', data.other_school_type.trim())
      if (data.established_year?.trim()) fd.append('established_year', data.established_year.trim())
      ...
      if (data.address_line_2?.trim()) fd.append('address_line_2', data.address_line_2.trim())
      ...
      if (data.medium_of_instruction?.trim()) fd.append('medium_of_instruction', data.medium_of_instruction.trim())
      if (data.other_medium_of_instruction?.trim()) fd.append('other_medium_of_instruction', data.other_medium_of_instruction.trim())
      if (data.classes_from?.trim()) fd.append('classes_from', data.classes_from.trim())
      if (data.classes_to?.trim())   fd.append('classes_to',   data.classes_to.trim())
      if (data.udise_code?.trim())   fd.append('udise_code',   data.udise_code.trim())
  ...
  if (submitted) {
    return <SuccessState response={submitted} principalEmail={principalEmail} />
  }
    ...
    <div className="mx-auto grid w-full max-w-[440px] gap-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-sm font-semibold text-foreground truncate">{stepInfo.title}</h2>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        {currentStep === 1 && <SchoolInfoStep register={register} errors={errors} watch={watch} setValue={setValue} />}
        {currentStep === 2 && <ContactStep register={register} errors={errors} />}
        {currentStep === 3 && <AcademicStep register={register} errors={errors} watch={watch} setValue={setValue} />}
        {currentStep === 4 && (
          <DocumentsStep
        ...
        {currentStep === 5 && <PrincipalStep register={register} errors={errors} watch={watch} setValue={setValue} />}
        ...
        <div className="flex gap-3 mt-6">
          ...
          {currentStep < 5 ? (
            ...
            <AuthButton type="submit" className="flex-1" disabled={isSubmitting}>
              {isSubmitting ? (
                <>
