...
import type { SchoolSearchItem, ClassCodeItem, StudentSearchItem } from '@/types/auth'
...
export function useSchoolSearch(debounceMs = 500) {
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .then((results) => { if (!cancelled) setSchools(results) })
      .catch(() => { /* silently ignore — search errors don't block registration */ })
...
export function useSchoolClasses(schoolId: string | undefined) {
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .then((results) => { if (!cancelled) setClasses(results) })
...
export function useStudentSearch(schoolId: string | undefined, debounceMs = 500) {
  const [query, setQuery] = useState('')
  const debouncedQuery = useDebounce(query, debounceMs)
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .then((results) => { if (!cancelled) setStudents(results) })
      .catch(() => { /* silently ignore */ })
      .finally(() => { if (!cancelled) setIsSearching(false) })
    return () => { cancelled = true }
