...
const config: Record<AlertVariant, { icon: React.ElementType; classes: string }> = {
  error: { icon: AlertCircle, classes: 'bg-destructive/10 border-destructive/20 text-destructive' },
  danger: { icon: AlertCircle, classes: 'bg-destructive/10 border-destructive/20 text-destructive' }, // Alias for error
  success: { icon: CheckCircle2, classes: 'bg-green-500/10 border-green-500/20 text-green-600 dark:text-green-400' },
  warning: { icon: AlertTriangle, classes: 'bg-amber-500/10 border-amber-500/20 text-amber-600 dark:text-amber-400' },
...
export function Alert({ variant = 'info', title, children, className, onClose }: AlertProps) {
  ...
  return (
    <div className={cn('flex gap-3 rounded-lg border p-4 backdrop-blur-sm', classes, className)} role="alert">
